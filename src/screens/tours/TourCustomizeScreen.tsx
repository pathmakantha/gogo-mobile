...
import { tourStopRemovalToggled } from '../../store/slices/discoverSlice';

type Nav = NativeStackNavigationProp<HomeStackParamList, 'TourCustomize'>;
type Route = RouteProp<HomeStackParamList, 'TourCustomize'>;

export function TourCustomizeScreen() {
  ...
  const budget = useAppSelector(state => state.trip.setup.budget);
  ...
  function toggleStop(stopId: string) {
    dispatch(tourStopRemovalToggled({ tourId, stopId }));
  }
    ...
    <SafeAreaView className="flex-1 bg-light-bg dark:bg-deep-dark" edges={['top', 'bottom']}>
      <View className="flex-row items-center gap-3 px-5 pt-4 pb-2">
        ...
        <View className="flex-1">
          <Text className="font-sora-extrabold text-lg text-dark-green dark:text-white" numberOfLines={1}>
            {detail.title}
          </Text>
      ...
      <ScrollView className="flex-1 px-[18px]" contentContainerStyle={{ gap: 9, paddingTop: 6 }}>
        {detail.stops.map(stop =>
          stop.removed ? (
            ...
            >
              ...
              <Pressable onPress={() => toggleStop(stop.id)} hitSlop={8}>
      ...
      <View className="flex-row items-center justify-between gap-3 border-t border-card-border bg-white px-[18px] py-3.5 dark:border-deep-dark-border dark:bg-deep-dark-card">
        <View>
          ...
          <Text
            className={`font-manrope-bold text-[11px] ${delta >= 0 ? 'text-primary' : 'text-[#B04A4E]'}`}
          >
            {delta >= 0
              ? t('tourDetail.underBudget', { amount: delta })
              : t('tourDetail.overBudget', { amount: Math.abs(delta) })}
          </Text>
        </View>
        <Pressable
          onPress={() =>
            (navigation.getParent() as any)?.navigate('PlansTab', { screen: 'ItineraryDayTimeline' })
          }
          className="rounded-[13px] bg-primary px-[22px] py-[13px]"
        >
          <Text className="font-manrope-bold text-sm text-white">{t('tourDetail.addToTrip')}</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
